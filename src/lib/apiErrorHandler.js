...
async function clearCrmAuthCache() {
    const platformInfo = await chrome.storage.local.get('platform-info');
    const platformName = platformInfo['platform-info']?.platformName;
    const cleared = await authCore.clearLocalCrmAuthState({ platformName });
    if (cleared && onCrmAuthCacheCleared) {
...
async function handleCrmAuthRequiredResponse({ showUserNotification = true } = {}) {
    ...
    const cleared = await clearCrmAuthCache();
    if (!cleared) {
        return false;
    }
...
function registerAxiosCrmAuthInterceptor() {
...
export default {
    ...
    registerAxiosCrmAuthInterceptor,
};
