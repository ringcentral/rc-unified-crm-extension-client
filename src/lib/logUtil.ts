...
declare const RCAdapter: {
    getCallLog(options: UnknownRecord): Promise<unknown>;
};
...
function getAdditionalFieldDefaultValuesFromSetting({
    ...
    if (!!additionalFields && !!platform.settings && platform.settings.length > 0) {
        for (const field of additionalFields) {
            let defaultValueSetting: UnknownRecord | null = null;
            ...
            if (defaultValueSetting) {
                ...
                if (valueItem) {
                    result.push({ field: field.const, value: (userCore as any).getCustomSetting(userSettings, valueItem.id, valueItem.defaultValue).value });
                }
            }
        }
    }
...
async function logPageFormDataDefaulting({ platform, targetPage, caseType, logType }: UnknownRecord): Promise<UnknownRecord> {
...
function allowBullhornCustomNoteAction({ platformName, userSettings }: UnknownRecord): boolean {
...
function rawValueCompare(value1: unknown = '', value2: unknown = ''): boolean {
...
function findMatchingFieldOption(options: UnknownRecord[] | undefined, value: unknown): unknown | null {
...
function getExistingContacts(contactInfo: UnknownRecord[] = []): UnknownRecord[] {
...
function parseUtcDateParts({ year, month, day, hour = 0, minute = 0, second = 0, millisecond = 0 }: UnknownRecord): number | null {
...
function parseCompactDateValue(rawValue: string): number | null {
...
function parseNumericDateValue(rawValue: string): number | null {
...
function parseIsoLikeDateValue(rawValue: string): number | null {
...
function parseDotNetDateValue(rawValue: string): number | null {
    const match = rawValue.match(/^\/Date\((-?\d+)(?:[+-]\d{4})?\)\/$/);
    return match ? toValidTimestamp(Number(match[1])) : null;
}

function parseNamedDateValue(rawValue: string): number | null {
...
function parseContactDateValue(value: unknown): number | null {
...
function hasValidDateValue(value: unknown): boolean {
    if (value === undefined || value === null || String(value).trim() === '') {
        return false;
    }
...
function resolveEarliestCreatedContact(contactInfo: UnknownRecord[] = []): UnknownRecord {
...
function resolveMostRecentActivityContact(contactInfo: UnknownRecord[] = []): UnknownRecord | null {
...
async function addPendingRecordingSessionId({ sessionId }: UnknownRecord): Promise<void> {
    const { pendingRecordings } = await chrome.storage.local.get({ pendingRecordings: [] }) as UnknownRecord;
...
async function triggerPendingRecordingCheck({ serverUrl }: UnknownRecord): Promise<void> {
    let { pendingRecordings } = await chrome.storage.local.get({ pendingRecordings: [] }) as UnknownRecord;
    const removedPendingRecordings: unknown[] = [];
    if (pendingRecordings.length > 0) {
        for (const sessionId of pendingRecordings) {
            const callLog = await RCAdapter.getCallLog({ sessionId }) as UnknownRecord | null;
            if (callLog) {
                await logService.syncCallData({
                    serverUrl,
                    dataBody: callLog as any
                });
...
async function removePendingRecordingSessionId({ sessionId }: UnknownRecord): Promise<void> {
    let { pendingRecordings } = await chrome.storage.local.get({ pendingRecordings: [] }) as UnknownRecord;
    if (pendingRecordings.includes(sessionId)) {
...
async function cacheLogPageData({ id, manifest, logType, triggerType, platformName, direction, contactInfo, logInfo, loggedContactId }: UnknownRecord): Promise<void> {
    if(!manifest)
    {
...
async function getCachedLogPageData(): Promise<UnknownRecord | null> {
    const { cacheLogPageData } = await chrome.storage.local.get({ cacheLogPageData: null }) as UnknownRecord;
    return cacheLogPageData;
}

export {
    getLogConflictInfo,
    logPageFormDataDefaulting,
    getExistingContacts,
    parseContactDateValue,
    hasValidDateValue,
    resolveMostRecentActivityContact,
    resolveEarliestCreatedContact,
    addPendingRecordingSessionId,
    triggerPendingRecordingCheck,
    removePendingRecordingSessionId,
    cacheLogPageData,
    getCachedLogPageData,
};
