...
export async function onEvent({ data, manifest, platformInfo, platformName, platform }: EventOptions): Promise<void> {
  ...
  if (contact === 'newContact' && newContactName && newContactName.trim() !== '') {
    const ct = manifest.platforms[platformName as string]?.contactTypes || [];
    ...
    let selectedContact: UnknownRecord | null = null;
    if (contactIdToUse && contactIdToUse !== 'newContact') {
      ...
      if (matched && contactInfo && contactInfo.length > 0) {
        ...
        selectedContact = contactInfo.find((c: UnknownRecord) => c.id === contactIdToUse) ?? null;
        if (selectedContact) {
