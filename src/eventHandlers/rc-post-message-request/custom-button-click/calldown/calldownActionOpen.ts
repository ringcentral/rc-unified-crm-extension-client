...
export async function onEvent({ data, manifest, platformInfo, platformName, platform, listButtonItemId }: EventOptions): Promise<void> {
  ...
  if (item?.contactId && item?.phoneNumber) {
    ...
    if (matched) {
      const realContacts = (contactInfo || []).filter((c: UnknownRecord) => !c.isNewContact);
      const exact = realContacts.find((c: UnknownRecord) => c.id == item.contactId);
      if (exact) {
    ...
    if (matched) {
      ...
      const preferred = item?.contactId ? realContacts.find((c: UnknownRecord) => c.id == item.contactId) : null;
