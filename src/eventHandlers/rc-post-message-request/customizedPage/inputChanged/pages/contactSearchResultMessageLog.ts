...
export async function onEvent({ data, manifest, platformInfo, platformName, platform }: EventOptions): Promise<void> {
  void platformInfo;
  void platform;
  if (data.body.keys.some((k: string) => k === 'contactList')) {
    ...
    const initialLogPage = logPage.getLogPageRender({ ...cacheLogPageData, contactInfo: contactData.map(withoutNewContact) });
