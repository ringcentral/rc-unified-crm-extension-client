...
export async function onEvent({ data }: EventOptions): Promise<void> {
  ...
  if (data.loggedIn) {
    ...
    if (crmAuthed) {
      ...
      setInterval(async function () {
        userSettings = await userCore.refreshUserSettings({}) as UnknownRecord;
      ...
      try {
        ...
        if (apptCfg.supported) {
          ...
          const { userSettings: storedUserSettings } = await chrome.storage.local.get({ userSettings: {} }) as { userSettings: UnknownRecord };
  ...
  // -------Process RC login-------
  // ------------------------------
  let { rcLoginStatus } = await chrome.storage.local.get('rcLoginStatus') as { rcLoginStatus?: boolean };
  // case 1: fresh login
  ...
  const registeredVersionInfo = await chrome.storage.local.get('rc-crm-extension-version') as UnknownRecord;
