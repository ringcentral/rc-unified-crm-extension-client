...
import * as embeddableServices from '../service/embeddableServices';
...
async function bullhornHeartbeat({ platform }: UnknownRecord): Promise<void> {
    ...
    try {
        ...
        if (response.data.successful) {
            ...
            const serviceManifest = await (embeddableServices as any).getServiceManifest();
            (document.querySelector("#rc-widget-adapter-frame") as any).contentWindow.postMessage({
...
async function tryConnectToBullhorn({ platform }: UnknownRecord): Promise<void> {
    let authUri = null;
    let { crm_extension_bullhorn_user_urls } = await chrome.storage.local.get({ crm_extension_bullhorn_user_urls: null }) as UnknownRecord;
    if (crm_extension_bullhorn_user_urls?.oauthUrl) {
        ...
        const { data: crm_extension_bullhorn_user_urls } = await axios.get(`https://rest.bullhornstaffing.com/rest-services/loginInfo?username=${crm_extension_bullhornUsername}`) as { data: UnknownRecord };
...
export { bullhornHeartbeat, tryConnectToBullhorn };
export default {
    bullhornHeartbeat,
    tryConnectToBullhorn,
};
