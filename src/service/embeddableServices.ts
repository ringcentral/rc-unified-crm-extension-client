import userCoreBase from '../core/user';
...
const userCore = userCoreBase as UnknownRecord;
...
async function getServiceManifest() {
    const { isAdmin } = await chrome.storage.local.get({ isAdmin: false }) as UnknownRecord;
    const { userSettings } = await chrome.storage.local.get({ userSettings: {} }) as UnknownRecord;
    const { userPermissions } = await chrome.storage.local.get({ userPermissions: {} }) as UnknownRecord;
    ...
    const { myBannerDismissedDate } = await chrome.storage.local.get({ myBannerDismissedDate: 0 }) as UnknownRecord;
...
export default embeddableServices;
