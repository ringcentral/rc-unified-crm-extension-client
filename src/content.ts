...
const windowAny = window as UnknownRecord;
...
export async function checkUrlMatch({ type = 'quickAccessButton' }: UnknownRecord = {}) {
  try {
    const { allowEmbeddingForAllPages } = await chromeStorageLocal.get({ allowEmbeddingForAllPages: false });
    if (allowEmbeddingForAllPages) {
      return true;
    }
    const platformInfo = await chromeStorageLocal.get('platform-info');
    if (!isObjectEmpty(platformInfo)) {
      const { customCrmManifest } = await chromeStorageLocal.get({ customCrmManifest: null });
      ...
      const { userSettings } = await chromeStorageLocal.get('userSettings');
...
export function createC2DInstance({ rootNode, sharedWidget = null, matcherType, selectedRegion, c2dIgnoreSelector }: UnknownRecord) {
  let matcher;
  ...
  const c2dInstance = new windowAny.RingCentralC2D(options);
...
export async function initializeC2D() {
  ...
  const { userPermissions } = await chromeStorageLocal.get({ userPermissions: {} });

  // Store all C2D instances and observers
  windowAny.clickToDialInstances = windowAny.clickToDialInstances || [];
  windowAny.clickToDialObservers = windowAny.clickToDialObservers || [];
  windowAny.clickToDialShadowRootPollers = windowAny.clickToDialShadowRootPollers || [];

  // Get matcher type
  const { c2dMatcherType } = await chromeStorageLocal.get({ c2dMatcherType: 'libPhone' });
  const { selectedRegion } = await chromeStorageLocal.get({ selectedRegion: null });
  ...
  windowAny.clickToDialInject = createC2DInstance({
  ...
  windowAny.clickToDialInject.widget.update({ enableC2Text: userPermissions?.c2sms ?? false });
  windowAny.clickToDialInstances.push(windowAny.clickToDialInject);
  console.log(`[App Connect] C2D initialized for document.body`);

  initializeShadowRootSupport({
    createC2DInstance,
    sharedWidget: windowAny.clickToDialInject.widget,
    matcherType: c2dMatcherType,
    selectedRegion,
    c2dIgnoreSelector,
    onInstanceCreated: (instance) => windowAny.clickToDialInstances.push(instance),
    onObserverCreated: (observer) => windowAny.clickToDialObservers.push(observer),
    pollerStore: windowAny.clickToDialShadowRootPollers,
  });

  console.log(`[App Connect] C2D initialization complete. Found ${windowAny.clickToDialInstances.length} roots.`);
}
...
chrome.runtime.onMessage.addListener(
  function (request, sender, sendResponse) {
    ...
    if (request.action === 'pipedriveAltAuthDone') {
      ...
      (rcStepper as UnknownRecord).innerHTML = '(3/3) Setup finished. You can close this page now.';
    }
    // Unique: Bullhorn
...
async function fetchBullhornUserinfo() {
  ...
  let { crm_extension_bullhorn_user_urls } = await chromeStorageLocal.get({ crm_extension_bullhorn_user_urls: null });
...
export async function Initialize() {
  ...
  const { renderQuickAccessButton } = await chromeStorageLocal.get({ renderQuickAccessButton: true });
...
if (window.location.pathname === '/pipedrive-redirect') {
  ...
  const rcStepper = window.document.querySelector('#rc-stepper');
  (rcStepper as UnknownRecord).innerHTML = '(2/3) Please sign in on the extension with your RingCentral account. If nothing happens, please try refreshing this page and wait for a few seconds.';
}
...
export function registerInsightlyApiKey() {
  if (window.location.pathname === '/Users/UserSettings' && window.location.hostname.includes('insightly.com')) {
    const insightlyApiKey = (document.querySelector('#apikey') as UnknownRecord).innerHTML;
    const insightlyApiUrl = (document.querySelector('#apiUrl') as UnknownRecord).firstChild.innerHTML;
    sendMessageToExtension({
      type: 'insightlyAuth',
