...
function getAppointmentEditPageRender({
  ...
  const merged: UnknownRecord = { ...defaults, ...(initialFormData || {}) };
  if (statusVisible) {
...
async function saveAppointmentEdits({ manifest, jwtToken, formData }: UnknownRecord): Promise<unknown | null> {
...
export { getAppointmentEditPageRender, saveAppointmentEdits };

const appointmentEditPage = {
  getAppointmentEditPageRender,
  saveAppointmentEdits,
};
