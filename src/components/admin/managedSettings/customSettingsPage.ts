...
function getCustomSettingsPageRender({ crmManifest, adminUserSettings, userSettings }: UnknownRecord): UnknownRecord | null {
                        ...
                        page.schema.properties[setting.id] = {
                            ...
                            properties: {
                                ...
                                value: {
                                    ...
                                    items: {
                                        type: 'string',
                                        enum: setting.dynamicOptions ? userSettings?.[setting.id]?.options?.map((option: UnknownRecord) => option.id) : setting.options.map((option: UnknownRecord) => option.id),
                                        enumNames: setting.dynamicOptions ? userSettings?.[setting.id]?.options?.map((option: UnknownRecord) => option.name) : setting.options.map((option: UnknownRecord) => option.name)
                                    },
                                    uniqueItems: true
                                }
                            }
                        ...
                        page.schema.properties[setting.id] = {
                            ...
                            properties: {
                                ...
                                value: {
                                    type: 'string',
                                    title: setting.name,
                                    oneOf: setting.dynamicOptions ? userSettings?.[setting.id]?.options?.map((option: UnknownRecord) => ({
                                        const: option.id,
                                        title: option.name
                                    })) : setting.options.map((option: UnknownRecord) => ({
                                        const: option.id,
                                        title: option.name
                                    }))
                                }
    ...
    // Also surface overriding number format here (moved from Managed settings -> Call-pop)
...
export { getCustomSettingsPageRender };
export default {
    getCustomSettingsPageRender,
};
