...
function buildAdditionalFieldsSchema({ allAdditionalFields, contact, logInfo }: UnknownRecord) {
    const additionalFields: UnknownRecord = {};
    const additionalFieldsValue: UnknownRecord = {};
    const additionalWarningUISchemas: UnknownRecord = {};
...
function buildContactWarningField(contactList: UnknownRecord[], defaultContact: UnknownRecord): UnknownRecord {
...
function buildNewContactWidget(defaultContact: UnknownRecord, manifest: UnknownRecord, platformName: string): UnknownRecord {
    const newContactWidget: UnknownRecord = {
...
function buildSingleContactSection({ contactInfo, manifest, platformName, logInfo, useContactSearch, id, contactPhoneNumber }: UnknownRecord): UnknownRecord {
    ...
    let allAdditionalFields: UnknownRecord[] = manifest.platforms[platformName].page?.messageLog?.additionalFields ?? [];
    if (defaultContact.isNewContact) {
    ...
    const sectionUISchema: UnknownRecord = {
        id: { "ui:widget": "hidden" },
        warning: {
            "ui:field": "admonition",
            "ui:severity": "warning",
    ...
    const sectionFormData: UnknownRecord = {
        id,
