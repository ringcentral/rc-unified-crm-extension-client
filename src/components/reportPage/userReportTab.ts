...
function getUserReportTabRender({ page, userStats, userSettings, selectedRcExtension, rcExtensions }: UnknownRecord): UnknownRecord {
    ...
    let uiSchemaToAdd: UnknownRecord = {
        ...(rcExtensions?.length > 0 ? {
        rcExtensionList: {
