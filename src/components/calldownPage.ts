...
function getCalldownPageRender(): UnknownRecord {
...
async function getCalldownPageWithRecords({ manifest, filterName = '', filterStatus = 'All', searchWithFilters = {}, userSettings }: UnknownRecord): Promise<UnknownRecord> {
    ...
    try {
        ...
        let items: UnknownRecord[] = Array.isArray(data?.items) ? data.items : [];
        ...
        const idToContact = new Map<string, UnknownRecord>();
        try {
            ...
            const matcherRoot = (document.querySelector('#rc-widget-adapter-frame') as any)?.contentWindow?.phone?.contactMatcher?.data ?? {};
            for (const [phone, platformData] of Object.entries(matcherRoot) as Array<[string, UnknownRecord]>) {
        ...
        const enriched: UnknownRecord[] = items.map(i => {
        ...
        const filtered: UnknownRecord[] = normalizedSearch === ''
            ? enriched
            : enriched.filter(i => (
        ...
        try {
            ...
            if (activeContactIds.length > 0) {
                const updatedCache: UnknownRecord = {};
...
export { getCalldownPageRender, getCalldownPageWithRecords };

const calldownPage = {
    getCalldownPageRender,
    getCalldownPageWithRecords,
};

export default calldownPage;
